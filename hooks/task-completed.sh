...
# Fires when Antigravity (main agent or subagent) finishes its work.
...
for keyword in $keywords; do
  ...
  if [ -n "$task_id" ]; then
    trekker comment add "$task_id" -a "agent" -c "Auto-synced: completed via Stop hook. Antigravity task: $task_subject" 2>/dev/null || true
