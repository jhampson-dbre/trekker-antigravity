...
### Step 5: Mirror to task.md

After Trekker is set up, create matching todos in `<appDataDir>\brain\<conversation-id>\task.md` via `write_to_file`/`replace_file_content`:
```
Create a checklist task entry for each Trekker task:
- [ ] TREK-n: Task Title (Blocked by TREK-m)
```
...
```bash
trekker comment add TREK-n -a "agent" -c "Checkpoint:
...
# 5. Mirror to task.md with same structure via write_to_file/replace_file_content
```
