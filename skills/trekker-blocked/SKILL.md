...
**IMPORTANT**: `/trekker-blocked` is a skill (invoke via run_command / file edits), NOT a bash command. There is no `trekker blocked` CLI command. Use `trekker task update` as shown below.
...
```bash
# Add blocker comment
trekker comment add <task-id> -a "agent" -c "Blocked: <reason>"
