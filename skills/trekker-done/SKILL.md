...
**IMPORTANT**: `/trekker-done` is a skill (invoke via run_command / file edits), NOT a bash command. There is no `trekker done` CLI command. Use `trekker task update` as shown below.
...
3. Add detailed summary comment (author: `agent`)
...
trekker comment add <task-id> -a "agent" -c "Summary: <what was done>. Files: <modified files>. Changes: <specific changes>."
