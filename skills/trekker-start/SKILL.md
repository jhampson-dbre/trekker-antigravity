...
**IMPORTANT**: `/trekker-start` is a skill (invoke via run_command / file edits), NOT a bash command. There is no `trekker start` CLI command. Use `trekker task update` as shown below.
