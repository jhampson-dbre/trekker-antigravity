...
**CRITICAL: Trekker is your PRIMARY task tracker. It MUST be prioritized over Antigravity's built-in task.md.**

Trekker provides persistent task memory across sessions. Unlike task.md which is conversation-scoped, trekker stores tasks in a SQLite database that survives context resets.
...
## CRITICAL: Trekker Over task.md

```
Trekker = Source of Truth (survives context resets)
task.md = Secondary mirror (conversation-scoped only)
```

**ALWAYS:**
1. Create tasks in Trekker FIRST, then mirror to task.md
2. Update Trekker FIRST, then update task.md
...
## Trekker vs task.md Priority

| Aspect | Trekker (PRIMARY) | task.md (SECONDARY) |
|--------|-------------------|----------------------|
...
**Rule**: Trekker is the source of truth. task.md is a convenience mirror for the current session.
...
# Document progress
trekker comment add <task-id> -a "agent" -c "Progress: ..."

# Complete with summary
trekker comment add <task-id> -a "agent" -c "Summary: ..."
...
```bash
trekker comment add <task-id> -a "agent" -c "Checkpoint: done X. Next: Y. Files: a.ts, b.ts"
```
...
| `trekker comment add <id> -a "agent" -c "..."` | Add comment |
